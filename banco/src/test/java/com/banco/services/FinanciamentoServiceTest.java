...
public class FinanciamentoServiceTest {
    ...
    @Test
    public void testPagar5ParcelasDoFinanciamentoContaCorrenteFisica() {
        ...
        Conta contaAlison = abrirConta.abrirContaFisica(titularesContaAlison, TipoConta.CORRENTE, 50000.0);

        assertAll("Verificar detalhes da conta",
                () -> assertEquals("Alison Silva", contaAlison.getTitulares().get(0).getNomeCompeltoOuRazaoSocial(), "Nome do titular"),
                () -> assertEquals(50000.0, contaAlison.getSaldo(), "Saldo inicial"),
                () -> assertEquals(TipoConta.CORRENTE, contaAlison.getTipoConta(), "Tipo de conta")
        );

        FinanciamentoService financiamento = new FinanciamentoService();

        financiamento.novoFinanciamento(contaAlison, 50000.0, 12, 0.05);
        ...
        financiamento.pagarParcelasDoFinanciamento(contaAlison, contaAlison.getFinanciamentos().get(0).getNumeroFinanciamento(), "5");

        assertAll("Verificar detalhes do financiamento depois do pagamento de 5 parcelas",
                () -> assertEquals(5, contaAlison.getFinanciamentos().get(0).getQuantidadeParcelasPagas(), "Quantidade de parcelas pagas"),
                () -> assertEquals(21875.0, contaAlison.getFinanciamentos().get(0).getValorTotalPago(), "Valor das parcelas pagas")
        );
